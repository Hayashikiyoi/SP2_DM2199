...
void LoadScene::Render()
{
	...
	RenderMesh(meshList[GEO_NOWLOADING], false);
...
void LoadScene::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void LoadScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void LoadScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void LoadScene::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
